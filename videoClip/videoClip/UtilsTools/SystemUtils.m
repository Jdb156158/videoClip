...
+ (void)asyncPushAuthAlert:(NSString *)authName {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:authName message:NSLocalizedString(@"next_need_permission", nil) preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *okBtn = [UIAlertAction actionWithTitle:NSLocalizedString(@"title_cancel", nil) style:UIAlertActionStyleDefault handler:nil];
        UIAlertAction *go = [UIAlertAction actionWithTitle:NSLocalizedString(@"title_set", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
+ (void)openMailFeedback {
    ...
    NSString *recipients = @"support_mail@163.com";
    ...
    [mailUrl appendFormat:@"&subject=%@", NSLocalizedString(@"user_feedback_gif", nil)];
...
+ (void)clearImgCache {
    NSUInteger cache = [SDImageCache sharedImageCache].getSize ;
